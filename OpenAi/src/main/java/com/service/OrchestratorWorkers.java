...
public class OrchestratorWorkers {
     ...
     * @param chatClient The ChatClient to use for LLM interaction */
    ...
    public OrchestratorWorkers(ChatClient chatClient, String orchestratorPrompt, String workerPrompt) {
        // Assert is a debugging tool for verify assusmption
    ...
    public FinalResponse process(String taskDescription) {
        ...
        // OrchestratorResponse - It holds the final output + execution details produced by an orchestrator after coordinating multiple workers, tools, or workflow steps
        ...
        List<String> workerResponses = orchestratorResponse.tasks().stream().map(task -> this.chatClient.prompt()
                .user(u -> u.text(this.workerPrompt)
                        // .param() is used to add request parameters
                        .param("original_task", taskDescription)
                        .param("task_type", task.type())
                        //task.type() - Specifies the category or nature of the task.
                        // mental model It tells the orchestrator:
                        //Which worker or tool should handle this task
                        .param("task_description", task.description()))
                        // .description() - Provides the detailed instruction for the task
                .call()
                .content()).toList();
        ...
        // analysis() - How the system understood and processed the task before giving the final answer
    }
