...
@Service
public class ETLJSONTransformerService {

    private static final Logger logger = LoggerFactory.getLogger(ETLJSONTransformerService.class);
    ...
    public List<Document> apply(List<Document> documents) {
        for (Document document : documents) {
            ...
            /* PromptTemplate is used to used create dynamic prompts by inserting variables values into predefined prompt
            structure .
            KeywordMetadataEnricher.KEYWORDS_TEMPLATE - LLM to extract important keywords from document content and store them into in metadata
            KeywordMetadataEnricher.CONTEXT_STR_PLACEHOLDER -   The document content that will be inserted into the prompt before sending it to the LLM.

            putAll - copies all key-value pairs from one map into another map . */
